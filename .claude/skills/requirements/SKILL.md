...
```
v0.1  冷启动：profile + 行业痛点库 + 知识库 → 推演需求（大量 [假设待验证]）
v0.2  首访后：拜访记录 + v0.1 → 验证部分假设，补充新发现
...
| requirements.json 存在 + 用户提供新素材 | **B：迭代补充** | 增量更新需求 |
...
2. 接收用户提供的新素材
3. 判断是否需要读取 profile：
   - 用户消息明确提及"档案已更新/档案有更新" → 读取 `docs/customer/profile.json`，作为变更对比来源
   - 其他情况（用户提供拜访记录、沟通内容等新素材）→ **不读取 profile**，现有需求已包含档案沉淀信息

**步骤 2：提取新信号**

启动 extract-from-input Agent：
- 注入新素材
...
- 注入 `{extract_result}`：extract Agent 的输出
- 仅当步骤 1 判断需要时，注入 `{profile_summary}`：profile 精简摘要（档案更新场景）

Agent 内部完成合并（新增/验证/否定/修改），输出更新后的完整 JSON。
...
| 用户未提供素材（冷启动） | 全部走推演路径，所有需求标注 [假设待验证] |
